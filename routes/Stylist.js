...
router.post("/signup", async (req, res) => {
  const { firstName, lastName, emailAddress, businessName, phone, password } =
    req.body;
  ...
  try {
    ...
    if (existingStylist) {
      return res
        .status(400)
        .json({ message: "Stylist with this email already exists" });
    }
...
router.post("/add-options", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Options updated successfully", stylistId });
  } catch (error) {
...
router.post("/address", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Stylist address added successfully", salonId });
  } catch (error) {
    console.error("Error updating options:", error);
    res.status(500).json({ message: "Server error" });
  }
